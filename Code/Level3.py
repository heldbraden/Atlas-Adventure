import pygame
from Levels import world3_level1
...
def playLvl3():
    ...
    class WorldCreation():    
        ...
        def reset(self, data):
            ...
            #Tiles
            grass = pygame.image.load('../Graphics/tundraMid.png')
            dirt = pygame.image.load('../Graphics/tundraCenter.png')
            hillUp = pygame.image.load('../Graphics/tundraHillLeft.png')
            hillUpDirt = pygame.image.load('../Graphics/tundraHillLeft2.png')
            #water = pygame.image.load('../Graphics/liquidWaterTop_mid.png')
            bridge = pygame.image.load('../Graphics/tundraHalf.png')
            #coins = pygame.image.load('../Graphics/coinGold.png')
            #flag = pygame.image.load('../Graphics/flagRed.png')
            islandRight = pygame.image.load('../Graphics/tundraCliffRight.png')
            ...
            for row in data:
                ...
                for tile in row:
                    ...
                    if tile == 52:
                        waterTile = WaterDeep(colCount * tileSize, rowCount * tileSize)
                        waterGroup.add(waterTile)
                    if tile == 8:
                        ...
                        self.tileList.append(tile)
                    if tile == 83:
                        coinTile = Tree(colCount * tileSize, rowCount * tileSize)
    ...
    class Water(pygame.sprite.Sprite):
        def __init__(self, x, y):
            ...
            img = pygame.image.load('../Graphics/iceWaterMid.png')
            self.image = pygame.transform.scale(img, (tileSize, tileSize))
            self.rect = self.image.get_rect()
            self.rect.x = x
            self.rect.y = y

    class WaterDeep(pygame.sprite.Sprite):
        def __init__(self, x, y):
            pygame.sprite.Sprite.__init__(self)
            img = pygame.image.load('../Graphics/iceWaterDeep.png')
            self.image = pygame.transform.scale(img, (tileSize, tileSize))
            self.rect = self.image.get_rect()
            self.rect.x = x
            self.rect.y = y
    ...
    class Tree(pygame.sprite.Sprite):
        def __init__(self, x, y):
            pygame.sprite.Sprite.__init__(self)
            img = pygame.image.load('../Graphics/pineSapling.png')
            self.image = pygame.transform.scale(img, (tileSize, tileSize))
            self.rect = self.image.get_rect()
            self.rect.x = x
            self.rect.y = y
    ...
    world1 = WorldCreation(world3_level1)
